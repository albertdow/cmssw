...
void PFJetsTauOverlapRemoval::produce(edm::StreamID iSId, edm::Event& iEvent, const edm::EventSetup& iES) const
{
  ...
  const Provenance& prov = iEvent.getProvenance(PFJets.id());

  if(prov.productInstanceName() == "" || prov.productInstanceName() == "TwoJets"){
  ...
  else if(prov.productInstanceName() == "ThreeJets" && PFJets->size() > 2){
    for(unsigned int iTau = 0; iTau < taus.size(); iTau++){  
